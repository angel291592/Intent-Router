...
- 要你回答的更少，而不是更多。在任何问题到达你之前，它先去你的仓库、工单系统或文档里查一遍，
  四十六问的盘问于是被压缩成唯一一个真正需要你拍板的问题——而且那一个答案会落进交付的代码里：
  交付对照中，装了 skill 的每次运行只问一个问题，3/3 的交付都写明了缓存失败策略，不装的 5 次交付
  一次都没有写（[报告](evals/reports/2026-09-24-delivery-opencode.md)）。
